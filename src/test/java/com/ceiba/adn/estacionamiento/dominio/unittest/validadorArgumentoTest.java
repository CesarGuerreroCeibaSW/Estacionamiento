...
import com.ceiba.adn.estacionamiento.dominio.entidad.ValidaArgumento;
...
public class validadorArgumentoTest {
	...
	public void validadorArgumentoVacio() {
		try {
			// act
			ValidaArgumento.validarRequerido(VACIO, PLACA_VACIA);
			fail();
	...
	public void argumentValidatorIsNull() {
		try {
			// act
			ValidaArgumento.validarRequerido(null, PLACA_VACIA);
			fail();
		} catch (ArgumentoExcepcion e) {
			// assert
			assertEquals(e.getMessage(), PLACA_VACIA);
	...
	public void argumentValidatorDateIsNull() {
		try {
			// act
			ValidaArgumento.validarFechaRequerida(null,FECHA_VACIA);
			fail();
		} catch (ArgumentoExcepcion e) {
			// assert
	...
	public void argumentValidatorInvalidTypeVehicle() {
		try {
			// act
			ValidaArgumento.validarTipoVehiculo(BUS,TIPO_VEHICULO_INVALIDO);
			fail();
